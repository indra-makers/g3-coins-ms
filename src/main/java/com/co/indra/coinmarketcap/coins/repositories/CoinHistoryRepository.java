...
public class CoinHistoryRepository {
   ...
   public Page<CoinHistory> findAllCoinsHistoryPage(Pageable pageable) {
      Sort.Order order = !pageable.getSort().isEmpty() ? pageable.getSort().toList().get(0) : Sort.Order.by("date_reg");
   ...
   public List<CoinHistory> findByAllAttributes(String symbol, Long idCoin, Double high, Double low, Double closePrice,
