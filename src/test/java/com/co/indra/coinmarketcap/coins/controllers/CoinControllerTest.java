...
public class CoinControllerTest {
   ...
   public void createBasicCoinHappyPath() throws Exception {
      MockHttpServletRequestBuilder request = MockMvcRequestBuilders
            .post(Routes.COINS_PATH).content("{\r\n" + "    \"symbol\": \"TPU\",\r\n"
                  + "    \"nameCoin\": \"PruebaCoin\",\r\n" + "    \"icon\": \"ICON\"\r\n" + "}")
      ...
      List<Coin> coins = coinRepository.findBySymbol("TPU");
      Assertions.assertEquals(1, coins.size());

      Coin CoinToAssert = coins.get(0);

      Assertions.assertEquals("PruebaCoin", CoinToAssert.getNameCoin());
      Assertions.assertEquals("ICON", CoinToAssert.getIcon());
   }
}
